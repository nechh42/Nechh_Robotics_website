...
class CandleManager:
    ...
    def add_symbol(self, symbol: str):
        """Dinamik coin ekleme — candle geçmişini indir"""
        ...
        try:
            ...
            logger.info(f"[CANDLES] Dynamic add: {symbol} ({len(self.candles[symbol])} candles)")
        except Exception as e:
            logger.error(f"[CANDLES] Dynamic add failed {symbol}: {e}")